...
extension CodexService {
    ...
    // Refreshes only the bridge package version state so bridge updates stay independent from GPT UX.
...
extension CodexService {
    ...
    private func minimumBridgePackageUpdatePrompt(currentVersion: String?) -> CodexBridgeUpdatePrompt {
        ...
        if let currentVersion = currentVersion?.trimmingCharacters(in: .whitespacesAndNewlines),
           !currentVersion.isEmpty {
            message =
                "This computer bridge is running bridge \(currentVersion), but this iPhone app requires bridge \(CodexService.minimumSupportedBridgePackageVersion) or newer. Update the npm package on your computer, then reconnect."
        } else {
            message =
                "This computer bridge is too old for this version of Dexora iPhone. Update the bridge npm package on your computer to \(CodexService.minimumSupportedBridgePackageVersion) or newer, then reconnect."
        }

        return CodexBridgeUpdatePrompt(
            title: "Update the bridge on your computer to reconnect",
            message: message,
    ...
    private func availableBridgePackageUpdatePrompt(
        ...
        CodexBridgeUpdatePrompt(
            title: "A newer bridge update is available on your computer",
            message: "This computer bridge is running \(currentVersion), and npm now has \(latestVersion). Update the package on your computer when you're ready, then reconnect to start using the newer build.",
            command: minimumBridgePackageUpdateCommand
        )
    }
    ...
    private func forcedBridgePackageUpdatePrompt(currentVersion: String) -> CodexBridgeUpdatePrompt {
        CodexBridgeUpdatePrompt(
            title: "Update the bridge on your computer to reconnect",
            message: "This computer bridge is running \(currentVersion). Update the bridge CLI on your computer to \(forcedBridgeUpgradeTargetVersion), then reconnect.",
    ...
    func isExpectedGPTLoginCallbackURL(_ url: URL) -> Bool {
        guard let callbackScheme = Bundle.main.object(
            forInfoDictionaryKey: "DEXORA_CHATGPT_CALLBACK_SCHEME"
        ) as? String else {
            return false
        }
