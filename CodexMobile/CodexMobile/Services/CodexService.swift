...
final class CodexService {
    ...
    static let relaySessionIdFallbackDefaultsKey = "codex.reconnectFallback.relaySessionId"
    static let relayURLFallbackDefaultsKey = "codex.reconnectFallback.relayURL"
    static let relayMacDeviceIdFallbackDefaultsKey = "codex.reconnectFallback.relayMacDeviceId"
    static let relayMacIdentityPublicKeyFallbackDefaultsKey = "codex.reconnectFallback.relayMacIdentityPublicKey"
    static let relayProtocolVersionFallbackDefaultsKey = "codex.reconnectFallback.relayProtocolVersion"
    static let relayLastAppliedBridgeOutboundSeqFallbackDefaultsKey = "codex.reconnectFallback.relayLastAppliedBridgeOutboundSeq"
    static let trustedMacRegistryFallbackDefaultsKey = "codex.reconnectFallback.trustedMacRegistry"
    static let lastTrustedMacDeviceIdFallbackDefaultsKey = "codex.reconnectFallback.lastTrustedMacDeviceId"
    ...
    init(
        ...
        let secureTrustedMacRegistry = codexTrustedMacRegistryFromSecureStore()
        if secureTrustedMacRegistry.records.isEmpty,
           let fallbackData = defaults.data(forKey: Self.trustedMacRegistryFallbackDefaultsKey),
           let fallbackRegistry = try? decoder.decode(CodexTrustedMacRegistry.self, from: fallbackData) {
            self.trustedMacRegistry = fallbackRegistry
            SecureStore.writeCodable(fallbackRegistry, for: CodexSecureKeys.trustedMacRegistry)
        } else {
            self.trustedMacRegistry = secureTrustedMacRegistry
        }
        self.lastTrustedMacDeviceId = SecureStore.readString(for: CodexSecureKeys.lastTrustedMacDeviceId)
            ?? defaults.string(forKey: Self.lastTrustedMacDeviceIdFallbackDefaultsKey)
        if let lastTrustedMacDeviceId {
            SecureStore.writeString(lastTrustedMacDeviceId, for: CodexSecureKeys.lastTrustedMacDeviceId)
        }
        ...
        if let rawLastAppliedSeq = SecureStore.readString(for: CodexSecureKeys.relayLastAppliedBridgeOutboundSeq)
            ?? defaults.string(forKey: Self.relayLastAppliedBridgeOutboundSeqFallbackDefaultsKey),
        ...
        if let relayMacIdentityPublicKey {
            SecureStore.writeString(relayMacIdentityPublicKey, for: CodexSecureKeys.relayMacIdentityPublicKey)
        }
        SecureStore.writeString(String(relayProtocolVersion), for: CodexSecureKeys.relayProtocolVersion)
        SecureStore.writeString(String(lastAppliedBridgeOutboundSeq), for: CodexSecureKeys.relayLastAppliedBridgeOutboundSeq)
        persistRelaySessionFallback()
        persistTrustedMacRegistryFallback()
    ...
    // Mirrors non-secret reconnect metadata outside Keychain so self-host builds can recover
    // from transient Keychain read failures without losing an otherwise trusted Mac pairing.
    func persistRelaySessionFallback() {
        if let relaySessionId = normalizedRelaySessionId {
            defaults.set(relaySessionId, forKey: Self.relaySessionIdFallbackDefaultsKey)
        } else {
            defaults.removeObject(forKey: Self.relaySessionIdFallbackDefaultsKey)
        }

        if let relayURL = normalizedRelayURL {
            defaults.set(relayURL, forKey: Self.relayURLFallbackDefaultsKey)
        } else {
            defaults.removeObject(forKey: Self.relayURLFallbackDefaultsKey)
        }

        if let relayMacDeviceId = normalizedRelayMacDeviceId {
            defaults.set(relayMacDeviceId, forKey: Self.relayMacDeviceIdFallbackDefaultsKey)
        } else {
            defaults.removeObject(forKey: Self.relayMacDeviceIdFallbackDefaultsKey)
        }

        if let relayMacIdentityPublicKey = normalizedRelayMacIdentityPublicKey {
            defaults.set(relayMacIdentityPublicKey, forKey: Self.relayMacIdentityPublicKeyFallbackDefaultsKey)
        } else {
            defaults.removeObject(forKey: Self.relayMacIdentityPublicKeyFallbackDefaultsKey)
        }

        defaults.set(String(relayProtocolVersion), forKey: Self.relayProtocolVersionFallbackDefaultsKey)
        defaults.set(
            String(lastAppliedBridgeOutboundSeq),
            forKey: Self.relayLastAppliedBridgeOutboundSeqFallbackDefaultsKey
        )
    }

    func clearRelaySessionFallback() {
        defaults.removeObject(forKey: Self.relaySessionIdFallbackDefaultsKey)
        defaults.removeObject(forKey: Self.relayURLFallbackDefaultsKey)
        defaults.removeObject(forKey: Self.relayMacDeviceIdFallbackDefaultsKey)
        defaults.removeObject(forKey: Self.relayMacIdentityPublicKeyFallbackDefaultsKey)
        defaults.removeObject(forKey: Self.relayProtocolVersionFallbackDefaultsKey)
        defaults.removeObject(forKey: Self.relayLastAppliedBridgeOutboundSeqFallbackDefaultsKey)
    }

    func persistTrustedMacRegistryFallback() {
        if trustedMacRegistry.records.isEmpty {
            defaults.removeObject(forKey: Self.trustedMacRegistryFallbackDefaultsKey)
        } else if let data = try? encoder.encode(trustedMacRegistry) {
            defaults.set(data, forKey: Self.trustedMacRegistryFallbackDefaultsKey)
        }

        if let lastTrustedMacDeviceId = normalizedLastTrustedMacDeviceId {
            defaults.set(lastTrustedMacDeviceId, forKey: Self.lastTrustedMacDeviceIdFallbackDefaultsKey)
        } else {
            defaults.removeObject(forKey: Self.lastTrustedMacDeviceIdFallbackDefaultsKey)
        }
    }
