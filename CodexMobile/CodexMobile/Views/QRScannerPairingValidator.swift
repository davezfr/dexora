...
func validatePairingQRCode(_ code: String, now: Date = Date()) -> QRScannerPairingValidationResult {
        ...
        guard payload.v == codexPairingQRVersion else {
            return .bridgeUpdateRequired(
                makeScannerBridgeUpdatePrompt(
                    message: "This QR code was generated by a different bridge package version. Update the package on your computer to the latest release before scanning a new QR code."
                )
            )
        }
    ...
    if looksLikeRemodexPairingPayload(data) {
        return .bridgeUpdateRequired(
            makeScannerBridgeUpdatePrompt(
                message: "This QR code looks like it came from an older bridge. Update the npm package on your computer to the latest release before scanning a new QR code."
            )
        )
    }

    return .scanError("Not a valid secure pairing code. Make sure you're scanning a QR from the latest computer bridge.")
}
