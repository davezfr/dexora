// FILE: AboutDexoraView.swift
// Purpose: Full-screen guide explaining how Dexora works, styled as a blog page.
// Layer: View
// Exports: AboutDexoraView

import SwiftUI

struct AboutDexoraView: View {
    ...
    @ViewBuilder private var attributionSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            sectionTitle("Open Source")

            bodyText("Dexora is an unofficial self-host beta built from the open-source Remodex project under the Apache 2.0 license.")
    ...
    @ViewBuilder private var architectureDiagram: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(spacing: 0) {
                diagramStep(from: "Dexora iOS", to: "Bridge (Mac)", via: "WebSocket")
    ...
    @ViewBuilder private var desktopSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            calloutCard(
                ...
                color: .blue,
                text: "The desktop app doesn't live-reload external writes. Use the desktop app handoff button in Dexora to continue the current thread on your computer."
            )
        }
    }
