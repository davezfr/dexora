...
func dexoraNormalizedCreatedBranchName(_ rawName: String) -> String {
    let trimmedName = rawName.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmedName.isEmpty else { return "" }
    if trimmedName.hasPrefix("dexora/") {
        return trimmedName
    }
    return "dexora/\(trimmedName)"
}

// Leaves "open elsewhere" branches selectable so the caller can surface the right alert or git error.
func dexoraCurrentBranchSelectionIsDisabled(
    branch: String,
...
func dexoraSelectableDefaultBranch(
...
struct TurnGitBranchSelector: View, Equatable {
    ...
    private var localDefaultBranch: String? {
        ...
        return dexoraSelectableDefaultBranch(
...
struct TurnGitBranchPickerSheet: View {
    ...
    private var isNewBranchNameValid: Bool {
        ...
        guard !trimmed.isEmpty, trimmed != "dexora/" else { return false }
        return true
    }
    ...
    private var suggestedCreateBranchName: String? {
        ...
        let candidate = dexoraNormalizedCreatedBranchName(searchText)
    ...
    var body: some View {
        List {
            Section(sectionTitle) {
                if showsDefaultBranchRow, let defaultBranch {
                    let isDisabled = dexoraCurrentBranchSelectionIsDisabled(
            ...
            if allowsSelectingCurrentBranch {
                Section {
                    ...
                    Button {
                        let fromSearch = dexoraNormalizedCreatedBranchName(searchText)
                        newBranchName = fromSearch.isEmpty ? "dexora/" : fromSearch
        ...
        .alert("New branch", isPresented: $isShowingCreateBranchPrompt) {
            TextField("dexora/my-feature", text: $newBranchName)
            Button("Cancel", role: .cancel) {
                newBranchName = ""
            }
            Button("Create") {
                let branchName = dexoraNormalizedCreatedBranchName(newBranchName)
...
#Preview("Branch Picker") {
    TurnGitBranchPickerSheet(
        branches: [
            "feature/auth-flow",
            "feature/dark-mode",
            "dexora/onboarding-v2",
            "dexora/sidebar-refactor",
            "fix/crash-on-launch",
        ...
        selectedBranch: "dexora/onboarding-v2",
        defaultBranch: "main",
        currentBranch: "dexora/onboarding-v2",
