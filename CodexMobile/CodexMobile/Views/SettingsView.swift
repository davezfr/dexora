...
private struct SettingsSubscriptionCard: View {
    ...
    var body: some View {
        #if SELF_HOSTED
        SettingsCard(title: "Self-Hosted Access") {
            HStack {
                Text("Status")
                Spacer()
                Text("Active")
            ...
            Text("This private build uses local self-host access and does not contact RevenueCat.")
                .font(AppFont.caption())
                .foregroundStyle(.secondary)
        }
        #else
        SettingsCard(title: "Dexora Pro") {
            HStack {
                Text("Status")
                Spacer()
...
private struct SettingsBridgeVersionCard: View {
    ...
    private var guidanceText: String? {
        ...
        if installedVersion.compare(latestVersion, options: .numeric) == .orderedAscending {
            return "A newer bridge package is available on npm."
        }
