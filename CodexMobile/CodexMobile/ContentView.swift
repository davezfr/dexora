...
struct ContentView: View {
    ...
    private var rootContentWithLifecycleObservers: some View {
        rootContent
            // Resume saved-pairing recovery once onboarding is done, or when a saved pairing survived onboarding state loss.
            .task {
                guard shouldUseMainReconnectFlow, !isShowingManualScanner else {
    ...
    private var rootContentWithPresentations: some View {
        rootContentWithLifecycleObservers
            ...
            .fullScreenCover(isPresented: $isShowingReviewDemo) {
                ReviewDemoView {
    ...
    // A saved pairing is stronger than the onboarding flag. Self-host upgrades can keep
    // Keychain pairing metadata while losing UserDefaults, so don't block reconnect behind onboarding.
    private var shouldUseMainReconnectFlow: Bool {
        hasSeenOnboarding || codex.hasReconnectCandidate
    }
    ...
    private func openReviewDemo() {
        isShowingReviewDemo = true
    }
